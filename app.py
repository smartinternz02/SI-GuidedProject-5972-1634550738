...
@app.route('/')
def landingpage():
    img_url=url_for('static',filname='images/hello.png')
    print(img_url)
    flag=0
...
def predict():
    ...
    if request.method =='POST':
        ...
        for label in cols_target:
            filename=str(label+'model.sav')
            filename
            model = pickle.load(open(filename,'rb'))
            print('...Processing {}'.format(label))
            user_y_prob = model.predict_prob(user_features)[:,1]
        ...
        if not len(final):
            ...
            img_url = url_for('static',filname='images/happy.png')
            flag = 1
            print(img_url)
        else:
            text = "The comment is"
